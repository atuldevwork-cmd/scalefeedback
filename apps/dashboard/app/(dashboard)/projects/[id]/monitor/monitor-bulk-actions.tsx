...
export function MonitorBulkActionsBar({ projectId, issueIds, onDone }: Props) {
  ...
  function applyStatus() {
    startTransition(async () => {
      await Promise.all(
        issueIds.map((id) =>
          fetch(`/api/monitor/${projectId}/issues/${id}`, {
            method: 'PATCH',
      ...
      toast(`${count} issue${count > 1 ? 's' : ''} updated to ${status}`);
      onDone();
      router.refresh();
    });
  }
  ...
  function bulkDelete() {
    ...
    startDeleteTransition(async () => {
      await fetch(`/api/monitor/${projectId}/issues`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ids: issueIds }),
      });
      toast(`${count} issue${count > 1 ? 's' : ''} deleted`);
      onDone();
      router.refresh();
    });
  }
  ...
  function sendToFeedback() {
    startSendTransition(async () => {
      ...
      if (!res.ok) {
        toast('Failed to send issues to feedback', 'error');
        return;
      }
      toast(`${count} issue${count > 1 ? 's' : ''} sent to feedback`);
      onDone();
      router.push(`/projects/${projectId}`);
    });
  }

  const busy = isPending || isDeleting || isSending;
  ...
  return (
    <>
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 bg-[#1a0a2e] text-white pl-3 pr-2 py-2 rounded-2xl shadow-2xl border border-white/10">
        ...
        <button
          onClick={sendToFeedback}
          disabled={busy}
          title="Create feedback items from these issues so they enter your fix workflow"
          className="flex items-center gap-1 text-white/80 hover:text-white text-sm font-medium transition-colors disabled:opacity-60 px-1"
        >
          <span className="material-symbols-outlined text-[16px]">forward_to_inbox</span>
          {isSending ? 'Sending…' : 'Send to feedback'}
        </button>
        ...
        <div className="fixed inset-0 z-[60] bg-black/40 backdrop-blur-sm flex items-center justify-center p-6">
          <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-sm flex flex-col items-center gap-4">
            ...
            <div className="text-center">
              <p className="font-semibold text-[#111111] text-lg">Delete {count} issue{count > 1 ? 's' : ''}?</p>
