...
export default function ContactPage() {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      {/* Hero */}
      <section className="bg-gradient-to-b from-[#f5eeff] to-white pt-16 pb-12 text-center px-4">
        <span className="inline-block text-[11px] font-semibold tracking-widest text-[#ff724f] uppercase mb-4">Contact Us</span>
        <h1 className="text-4xl sm:text-5xl font-extrabold text-[#300a46] mb-4 leading-tight">
      ...
      <section className="flex-1 max-w-6xl mx-auto w-full px-4 sm:px-6 py-12 grid md:grid-cols-[1fr_380px] gap-12">
        ...
        <div>
          {status === 'sent' ? (
            <div className="flex flex-col items-center justify-center h-full text-center py-20 gap-5">
              ...
              <h2 className="text-2xl font-bold text-[#300a46]">Message sent!</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="grid sm:grid-cols-2 gap-5">
                <div>
                  <label className="block text-sm font-medium text-[#300a46] mb-1.5">Name <span className="text-red-400">*</span></label>
                  <input
                    type="text"
                    value={form.name}
                    onChange={set('name')}
                    ...
                    className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#300a46]/20 focus:border-[#300a46]/40"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-[#300a46] mb-1.5">Email <span className="text-red-400">*</span></label>
                  <input
                    type="email"
                    value={form.email}
                    onChange={set('email')}
              ...
              <div>
                <label className="block text-sm font-medium text-[#300a46] mb-1.5">Topic</label>
                <select
                  value={form.topic}
                  onChange={set('topic')}
                  className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#300a46]/20 focus:border-[#300a46]/40 bg-white text-slate-700"
                >
              ...
              <div>
                <label className="block text-sm font-medium text-[#300a46] mb-1.5">Message <span className="text-red-400">*</span></label>
                <textarea
                  value={form.message}
                  ...
                  className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#300a46]/20 focus:border-[#300a46]/40 resize-none"
                />
              </div>
              ...
              <button
                ...
                className="w-full bg-[#300a46] hover:bg-[#4a1566] text-white font-semibold py-3 rounded-xl text-sm transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
              >
        ...
        <div className="space-y-6">
          <div className="bg-[#f9f7fc] rounded-2xl p-6 border border-[#e8e0f0]">
            <h2 className="text-base font-semibold text-[#300a46] mb-5">Other ways to reach us</h2>
            <div className="space-y-5">
              {CONTACT_ITEMS.map((item) => (
                <div key={item.label} className="flex gap-3">
                  <div className="w-9 h-9 rounded-xl bg-[#300a46]/10 flex items-center justify-center shrink-0">
                    <span className="material-symbols-outlined text-[#300a46] text-[18px]">{item.icon}</span>
                  </div>
                  <div>
                    ...
                    {item.href ? (
                      ...
                      <p className="text-sm font-medium text-[#300a46]">{item.value}</p>
                    )}
          ...
          <div className="bg-[#fff3f0] rounded-2xl p-6 border border-[#ffddd7]">
            <div className="flex items-start gap-3">
              <span className="material-symbols-outlined text-[#ff724f] text-[22px] shrink-0 mt-0.5">rocket_launch</span>
              <div>
                <p className="text-sm font-semibold text-[#300a46] mb-1">Ready to get started?</p>
                <p className="text-xs text-slate-500 mb-3">Sign up free and start collecting feedback in minutes — no credit card required.</p>
                <Link href="/signup" className="inline-block bg-[#ff724f] hover:bg-[#e8623f] text-white text-xs font-semibold px-4 py-2 rounded-lg transition-colors">
                  Start free trial →
                </Link>
              </div>
            </div>
      ...
      {/* Footer */}
      <footer className="bg-[#300a46] text-purple-300 py-12 mt-auto">
